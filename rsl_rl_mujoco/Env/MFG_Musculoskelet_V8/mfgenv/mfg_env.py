...
class MFG_Musculoskelet_V8(gym.Env):
    """
    ...
    def __init__(self, config_path: Optional[str] = None, **kwargs) -> None:
        ...
        **kwargs
        ...
        self.traj_manager = _global_manager
        self.ref_traj = refs(
            manager=self.traj_manager,
            increment=ref_increment
            )
        self.ref_traj.reset(phase=0, traj_id=0)
        self.update_traj_init_info()
    ...
    def reset(self, 
        ...
        options : Optional[dict]
            Unused placeholder.
        custom_init_state : Optional[dict]
            May contain:
              - 'per': float or list of floats → desired gait phase(s).
              - 'traj_id': int → specific trajectory index.
        ...
        info : dict
        {
          'seed': the seed used (or None),
          'traj_id': selected trajectory index,
          'phase':   selected phase (0–100),
          'speed':   reference speed (m/s),
          'foot_start': 0 for right, 1 for left
        }
        ...
        if custom_init_state is not None:
            if 'per' not in custom_init_state:
                raise KeyError("custom_init_state must contain 'per'")
            per = custom_init_state['per']
            traj_id = custom_init_state.get('traj_id', None)
            if isinstance(per, list):
                phase = float(self.ref_traj.rng.choice(per))
            else:
                phase = float(per)
        else:
            traj_id = None
        ...
        self.ref_traj.reset(traj_id=traj_id, phase=phase)
        self.update_traj_init_info()
        ...
        info = {
            ...
            'phase':     round(self.ref_traj.phase, 4),
            'speed':     round(self.ref_traj.speed, 4),
            'foot_start': self.ref_traj.foot_start # 1=left, 0=right
        }
        logger.debug(f"Environment reset | {info}")
    ...
    def update_traj_init_info(self) -> None:
        """
