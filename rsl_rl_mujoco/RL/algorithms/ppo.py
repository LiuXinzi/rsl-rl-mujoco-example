...
from ..utils import string_to_callable,AMPLoader
...
class PPO:
    ...
    def __init__(
        ...
        self.useAmp = amp_data is not None
        if self.useAmp:
    ...
    def update(self):
        if self.useAmp:
            return self.update_amp()
        else:
            return self.update_withoutamp()

    def update_amp(self):  # noqa: C901
        mean_value_loss = 0
        ...
        return loss_dict
    def update_withoutamp(self):  # noqa: C901
        mean_value_loss = 0
        mean_surrogate_loss = 0
        mean_entropy = 0
        # -- RND loss
